...
import { MdDirectionsCar, MdTune, MdSettingsInputComponent } from 'react-icons/md';
...
export default function VehicleDetail() {
  ...
  return (
        <><Navbar />
        <section className="container md:pb-24 pb-16 mt-20 ml-0">
          <section className="relative md:pb-24 pb-16 mt-20">
            <div className="container-fluid">
              {vehicle && vehicle.interiorImages && vehicle.interiorImages.length > 0 && (
                <div className="md:flex justify-start">
                  <div className="grid grid-cols-3 gap-4">
                    {vehicle.interiorImages.map((imageArray, index) => (
                      <VehicleImage key={index} images={imageArray} />
                    ))}
                  </div>
                </div>
              )}
                ...
                {/* Render vehicle description */}
                {vehicle && (
                    <div className="md:p-4 px-3 mt-4 mr-0 text-black">
                        <h3 className="text-xl font-medium">Description</h3>
                        <p>{vehicle.vehicleDescription}</p> {/* Utilize vehicleDescription here */}
                    </div>
                )}
    ...
    <ul className="list-disc pl-4">
      <li className="mb-1">Minor cosmetic imperfections are not repaired as they do not affect the performance, and reduces the cost of ownership.</li>
...
<div className="mb-4 bg-gray-100 p-4 rounded-lg">
  ...
  <ul>
    <li className="mb-1">Some parts have been repainted for better aesthetics. However, we assure the car is non-accidental.</li>
...
<div className="mb-4 bg-gray-100 p-4 rounded-lg">
  ...
  <ul>
    <li className="mb-1">Thoroughly tested and ready for the road.</li>
...
<div className="mb-4 bg-gray-100 p-4 rounded-lg">
  ...
  <p>Rear Right: 75%</p>
              ...
              <h4 className="text-2xl font-medium">{vehicle.brand} {vehicle.model}</h4>
              <ul className="py-6 flex items-center list-none">
                <li className="flex items-center lg:me-6 me-4">
                  <MdDirectionsCar className="lg:text-3xl text-2xl me-2 text-green-600" />
                  <span className="lg:text-xl">{vehicle.variant}</span>
                </li>
                <li className="flex items-center lg:me-6 me-4">
                  <MdSettingsInputComponent className="lg:text-3xl text-2xl me-2 text-green-600" />
                  <span className="lg:text-xl">{vehicle.transmissionType}</span>
                </li>
                <li className="flex items-center">
                  <MdTune className="lg:text-3xl text-2xl me-2 text-green-600" />
                  <span className="lg:text-xl">{vehicle.kilometresDriven} KM</span>
                </li>
              </ul>
                ...
                <div className="lg:w-1/2 md:w-1/2 p-1">
                  <div className="flex flex-wrap justify-start gap-4">
                    ...
                    <div className="text-black">
                      ...
                      <p className="text-slate-900 mt-4">{vehicle.ownerCity}</p>
                      <p className="text-slate-900 mt-4">{vehicle.bodyType}</p>
                      <p className="text-slate-900 mt-4">{vehicle.color}</p>
                    </div>
                  </div>
                  <div className="rounded-md bg-slate-50 dark:bg-slate-800 shadow dark:shadow-gray-700 mt-4">
                    <div className="p-4 flex justify-between items-center">
                      <h5 className="text-2xl font-medium">Price:</h5>
                      <div className="flex items-center">
                        <span className="text-xl font-medium">{vehicle.price && `${vehicle.price.currency} ${vehicle.price.value}`}</span>
                        <div className="bg-green-600/10 text-green-600 text-sm px-2.5 py-0.75 rounded h-6 ml-4">For Sale</div>
                      </div>
                    </div>
                  </div>
                </div>
