...
export default function ListSidebar() {
    ...
    const [price, setPrice] = useState({ value: '', currency: '' }); // Ensure price state is initialized correctly
            ...
            return (
                <>
                    <label className="text-green-500">
                        <b>RTO Code:</b>
                        <input type="text" value={rtoCode} onChange={(e) => setRtoCode(e.target.value)} required={vehicleType !== 'edrone' && vehicleType !== 'ecycle'} className="w-full sm:w-48 h-8 border rounded-sm py-1 px-2" />
                    </label><br/><br/>
                    <label className="text-green-500">
                        <b>Kilometres Driven:</b>
                        <input type="text" value={kilometresDriven} onChange={(e) => setKilometresDriven(e.target.value)} required={vehicleType !== 'edrone' && vehicleType !== 'ecycle'} className="w-full sm:w-48 h-8 border rounded-sm py-1 px-2" />
                    </label><br/><br/>
                </>
            );
        }
    ...
    const renderTransmissionTypeField = () => {
        ...
        return (
            <label className="text-green-500">
                ...
                <select value={transmissionType} onChange={(e) => setTransmissionType(e.target.value)} required={vehicleType !== 'ecycle' && vehicleType !== 'edrone'} className="w-full sm:w-48 h-8 border rounded-sm py-1 px-2">
    ...
    const renderBodyTypeField = () => {
        if (vehicleType === 'ecar') {
            return (
                <label className="text-green-500">
                    <b>Body Type:</b>
                    <select value={bodyType} onChange={(e) => setBodyType(e.target.value)} required={vehicleType === 'ecar'} className="w-full sm:w-48 h-8 border rounded-sm py-1 px-2">
    ...
    return (
        <>
            ...
            <section className="py-20 bg-gray-100" style={{ backgroundImage: "url('/images/bg/b17.jpg')" }}>
                <div className="container mx-auto px-4">
                    <div className="pt-16 flex justify-center items-center min-h-screen">
                        <div className="w-full sm:max-w-md">
                            <h1 className="text-center text-2xl mb-4"><b>Enter Vehicle details you want to sell</b></h1>
                            ...
                            {showPopup && (
                                <div className="bg-green-200 border-l-4 border-green-500 text-green-700 p-4 fixed bottom-0 right-0 mb-4 mr-4">
                                    Vehicle registered successfully!
                                </div>
                            )}
                            {error && (
                                <div className="bg-red-200 border-l-4 border-red-500 text-red-700 p-4 fixed bottom-0 right-0 mb-4 mr-4">
                                    {error}
                                </div>
                            )}
                        </div>
