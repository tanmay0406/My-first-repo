...
export default function Grid() {
    ...
    useEffect(() => {
        const fetchApprovedEdrones = async () => {
            try {
                const response = await fetch('http://localhost:5000/api/vehicles/approved/edrone');
            ...
            <div key={index} className="group rounded-xl bg-white dark:bg-slate-900 shadow hover:shadow-xl dark:hover:shadow-xl dark:shadow-gray-700 dark:hover:shadow-gray-700 overflow-hidden ease-in-out duration-500">
                <div className="p-4">
                    {edrone.frontImagesBase64 && edrone.frontImagesBase64.length > 0 && (
                        <img src={`data:image/jpeg;base64,${edrone.frontImagesBase64[0]}`} alt="Front View" className="h-40 w-auto" />
                    )}
                    <h3 className="text-xl font-semibold mt-2">{edrone.name}</h3>
                    <p className="text-gray-600">Model: {edrone.model}</p>
                    <p className="text-gray-600">Brand: {edrone.brand}</p>
                    <p className="text-gray-600">Location: {edrone.location}</p>
                    <p className="text-gray-600">Battery Power: {edrone.batteryPower}</p>
                    <p className="text-gray-600">Color: {edrone.color}</p>
                    <p className="text-gray-600">Price: {edrone.price.currency} {edrone.price.value}</p>
